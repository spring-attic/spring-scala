...
class JmsTemplate(val javaTemplate: JmsOperations) {
	...
	/**
	 * Send the given object to the default destination, converting the object to a JMS message with a configured
	 * MessageConverter.
	 *
	...
	/**
	 * Send the given object to the specified destination, converting the object to a JMS message with a configured
	 * MessageConverter.
	 *
	 * @param destination the destination to send this message to
	 * @param message the object to convert to a message
	...
	/**
	 * Send the given object to the default destination, converting the object to a JMS message with a configured
	 * MessageConverter. The function allows for modification of the message after conversion.
	 *
	 * @param message the object to convert to a message
	...
	def convertAndSend(message: Any)(function: Message => Message) {
		javaTemplate.convertAndSend(message, new MessagePostProcessor {
			def postProcessMessage(message: Message) = {
				function(message)
			}
		})
	}
	...
	def convertAndSend(destinationName: String, message: Any)(function: Message => Message) {
		javaTemplate.convertAndSend(destinationName, message, new MessagePostProcessor {
			def postProcessMessage(message: Message) = {
				function(message)
			}
		})
	 ...
	 * @note This will only work with a default destination specified!
	 * @note This method should be used carefully, since it will block the thread until the message becomes available
	 * or until the timeout value is exceeded.
	 */
	...
	/**
	 * Receive a message synchronously from the specified destination, but only wait up to a specified time for
	 * delivery.
	 *
	 ...
	 * Receive a message synchronously from the default destination, but only wait up to a specified time for delivery.
	 *
	 * @param messageSelector the JMS message selector expression.
	 ...
	 *
	 * @param destination the destination to receive a message from
	 * @param messageSelector the JMS message selector expression. See the JMS specification for a detailed
	 * definition of selector expressions.
	 * @return the message received by the consumer, or `None` if the timeout expires
	 * @note This method should be used carefully, since it will block the thread until the message becomes available
	 * or until the timeout value is exceeded.
	 */
	...
	//-------------------------------------------------------------------------
	/**
	 * Receive a message synchronously from the default destination, but only wait up to a specified time for
	 * delivery. Convert the message into an object with a configured MessageConverter.
	 *
	 ...
	 * or until the timeout value is exceeded.
	 */
	def receiveSelectedAndConvert(destination: Destination, messageSelector: String): Option[AnyRef] = {
	...
	/**
	 * Browse messages in a JMS queue. The function gives access to the JMS Session and QueueBrowser in order to
	 * browse the queue and react to the contents.
	 *
