...
class ConstructorAutowiredAnnotationBeanPostProcessor
  ...
  val autowiredAnnotationTypes = new ListBuffer[Class[_ <: Annotation]]

  autowiredAnnotationTypes.append(classOf[Autowired])
  try {
    val cl: ClassLoader = classOf[AutowiredAnnotationBeanPostProcessor].getClassLoader
    autowiredAnnotationTypes.append(cl.loadClass("javax.inject.Inject").asInstanceOf[Class[_ <: Annotation]])
  ...
  def isAutowiredClass(beanClass: Class[_]) = AnnotationUtils.getAnnotation(beanClass, classOf[Autowired]) != null
