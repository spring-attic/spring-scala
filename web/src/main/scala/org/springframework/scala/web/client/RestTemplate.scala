...
class RestTemplate(val javaTemplate: org.springframework.web.client.RestOperations =
	...
	def postForObject[T](url: String, request: Option[Any], uriVariables: Any*)
		...
		Option(
			javaTemplate.postForObject(url, request.orNull, responseType(manifest), asInstanceOfAnyRef(uriVariables)))
	}
	...
	/**
	 * Create a new resource by POSTing the given object to the URI template, and returns the response as {@link HttpEntity}.
	 *
	 * URI Template variables are expanded using the given map.
	 *
	 * The {@code request} parameter can be a {@link HttpEntity} in order to add additional HTTP headers to the
	 * request.
	 *
	 * @param url the URL
	 * @param request the Object to be POSTed
	...
	def execute[T](url: String, method: HttpMethod, uriVariables: Any*)
		...
		Option(javaTemplate.execute(url, method, functionToRequestCallback(requestFunction),
			functionToResponseExtractor(responseFunction), asInstanceOfAnyRef(uriVariables)))
	}
	...
	def execute[T](url: String, method: HttpMethod, uriVariables: Map[String, _])
	              ...
	              (responseFunction: ClientHttpResponse => T): Option[T] = {
		Option(javaTemplate.execute(url, method, functionToRequestCallback(requestFunction),
			functionToResponseExtractor(responseFunction), uriVariables.asJava))
	}
	...
	def functionToRequestCallback(function: ClientHttpRequest => Unit): RequestCallback =
		new RequestCallback {
